...
class UserController extends ResourceController
{
    ...
    public function updateGeneralInfo()
    {
        ...
        if (hasAccess(auth()->user(), ADMIN_ACCESS) || hasAccess(auth()->user(), TRANSFER_OWNERSHIP)) {
            ...
            foreach ($this->request->getVar('user') as $key => $value) {
                ...
                // if($key == 'api/player/update-general-info' || $key == 'lang'){
                //     continue;
                // }
        ...
        if (!$user) {
            ...
            if ($users->delete($user->id)) {
                ...
                $activity_data = [
                    'user_id'               => auth()->id(),
                    'activity_type_id'      => 3,      // deleted
                    'activity'              => 'deleted his/her account',
                    'old_data'              => auth()->id(),
                    'ip'                    => $this->request->getIPAddress()
                ];
                createActivityLog($activity_data);
                ...
                $response = [
                    "status"    => true,
                    "message"   => lang('Your account has been deleted successfully'),
                    "data"      => ['user_data' => $user]
                ];
            } else {
                $response = [
                    "status"    => false,
                    "message"   => lang('account not deleted'),
                    "data"      => []
                ];
            }
    ...
    public function updateUserStatus()
    {
        ...
        if (hasAccess(auth()->user(), ADMIN_ACCESS) || hasAccess(auth()->user(), TRANSFER_OWNERSHIP) || hasAccess(auth()->user(), EDITOR_ACCESS)) {
            ...
            if (($this->request->getVar("id") && count($this->request->getVar("id")) > 0) && !empty($this->request->getVar("status"))) {
                    ...
                    if ($user) {
                        ...
                        if ($builder->update()) {
                            ...
                            if($getEmailTemplate){
                                ...
                                $message = view('emailTemplateView' , ['message' => $content, 'disclaimerText' => getEmailDisclaimer($user->lang) ]);
                                // $message = view('emailTemplateView' , ['message' => $content]);
                            ...
                            $response = [
                                "status"    => false,
                                "message"   => 'Status Not updated due to some issue, please try again',
                                "data"      => []
                            ];
                        }
                    }
                ...
                $response = [
                    "status"    => false,
                    "message"   => 'Please provide valid user IDs and status',
                    "data"      => []
                ];
            }
        } else {
        ...
        if (! $this->validateData([], $validationRule)) {
            ...
            if ($is_uploaded && $is_uploaded['status'] == "success") {
                ...
                $activity_data = [
                    ...
                    'activity'              => 'uploaded profile image ' . $is_uploaded['fileName'],
                    'ip'                    => $this->request->getIPAddress()
                ];
                createActivityLog($activity_data);
        ...
        if (! $this->validateData([], $validationRule)) {
            ...
            if (! $cover_image->hasMoved()) {
                ...
                $activity_data = [
                    'user_id'               => auth()->id(),
                    'activity_type_id'      => 2,      // updated
                    'activity'              => 'uploaded cover image ' . $cover_image->getName(),
                    'ip'                    => $this->request->getIPAddress()
                ];
                createActivityLog($activity_data);
        ...
        if ($userData) {
            ...
            if ($res) {
                ...
                $activity_data = [
                    ...
                    'activity'              => 'removed profile image ' . $userData['meta_value'],
                    'ip'                    => $this->request->getIPAddress()
                ];
                createActivityLog($activity_data);
        ...
        if ($userData) {
            ...
            if ($res) {
                ...
                $activity_data = [
                    'user_id'               => auth()->id(),
                    'activity_type_id'      => 3,      // updated
                    'activity'              => 'removed cover image ' . $userData['meta_value'],
                    'ip'                    => $this->request->getIPAddress()
                ];
                createActivityLog($activity_data);
            ...
            if (!$this->validate($rules)) {
                ...
                if ($users->save($userData)) {
                    ...
                    $activity_data = [
                        ...
                        'activity'              => 'added representator user(' . $this->request->getVar("email") . ') ',
                        'ip'                    => $this->request->getIPAddress()
                    ];
                    createActivityLog($activity_data);
            ...
            if (!$this->validate($rules)) {
                ...
                if ($representatorInfo->syncPermissions($this->request->getVar("site_role"))) {
                    ...
                    $activity_data = [
                        ...
                        'activity_type_id'      => 2,        // updated
                        'activity'              => 'Updated representator role',
                        'ip'                    => $this->request->getIPAddress()
                    ];
                    createActivityLog($activity_data);
            ...
            if ($userData) {
                ...
                if ($res) {
                    ...
                    $activity_data = [
                        ...
                        'activity_on_id'        => $userId,
                        'activity_type_id'      => 3,      // deleted 
                        'activity'              => 'deleted cover image ' . $userData['meta_value'],
                        'ip'                    => $this->request->getIPAddress()
                    ];
                    createActivityLog($activity_data);
            ...
            if (! $this->validateData([], $validationRule)) {
                ...
                if ($is_uploaded && $is_uploaded['status'] == "success") {
                    ...
                    $activity_data = [
                        ...
                        'activity_on_id'        => $userId,
                        'activity_type_id'      => 2,      // updated 
                        'activity'              => 'updated cover image ' . $is_uploaded['fileName'],
                        'ip'                    => $this->request->getIPAddress()
                    ];
                    createActivityLog($activity_data);
            ...
            if ($userData) {
                ...
                if ($res) {
                    ...
                    $activity_data = [
                        ...
                        'activity_on_id'        => $userId,
                        'activity_type_id'      => 3,      // deleted 
                        'activity'              => 'deleted profile image ' . $userData['meta_value'],
                        'ip'                    => $this->request->getIPAddress()
                    ];
                    createActivityLog($activity_data);
                ...
                if (!$clubHistory) {
                    ...
                    if ($userMetaObject->save($save_data)) {
                        ...
                        $activity_data = [
                            ...
                            'activity'              => 'added company history',
                            'ip'                    => $this->request->getIPAddress()
                        ];
                        createActivityLog($activity_data);
    ...
    // get total  Sales
    public function getGraphDataYearly($year = null)
    {
    ...
    public function getActivity()
    {
        ...
        $activities = $ActivityModel->getActivities($userId, $limit, $offset, $search);
    ...
    public function deleteActivity()
    {
        ...
        // $ids = $this->request->getPost('id');  // 'ids' can be a single ID or an array of IDs
        // if(isset($_POST['id']) && !empty($_POST['id'])){
        //    $ids = $_POST['id'];
        // }else if(!empty($this->request->getPost('id'))){
        ...
        $ids = $this->request->getVar("id");
        // echo '<pre>'; print_r($ids); die;
        // echo is_array($ids); die;
        if (is_array($ids)) {
            // If $ids is an array, delete multiple records
            ...
            // If $ids is a single value, delete one record
            if (!empty($ids) && $activityModel->where('user_id', $user_id)->where('id', $ids)->delete()) {
    ...
    public function downloadCsv()
    {
        ...
        if (isset($users['totalCount']) && $users['totalCount'] > 50) {
            ...
            if (isset($user) && !empty($user)) {
                ...
                $activity_data['activity'] = 'The ' . $filename . ' sent in Email successFully';
                createActivityLog($activity_data);
                return $this->respondCreated($response);
            }
        }
    ...
    public function downloadUserPdf($userId = null)
    {
        ...
        if (!empty($userId)) {
            ...
            * {box-sizing: border-box;}
            ...
            if (isset($userData['role']) && strtoupper($userData['role']) == '4') { // Player
                ...
                if (!empty($performanceDetail)) {
                    ...
                    foreach ($performanceDetail as $performance) {
                        ...
                        $team_img = '';
                        if(isset($performance['team_logo_path']) && !empty($performance['team_logo_path'])){
            ...
            $pdf_name = $pdf_name . '_' . $role_name;
            if (empty($pdf_name) || $pdf_name == '_') {
                $pdf_name = date('Y_M_D') . '_' . rand();
            }
            $filename = trim(strtolower($pdf_name)) . '.pdf';
            $folder_nd_file = 'uploads/exports/' . $filename;
            try {
