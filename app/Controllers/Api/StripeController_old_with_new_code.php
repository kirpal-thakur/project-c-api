...
class StripeController extends ResourceController
{
    ...
    public function initController(\CodeIgniter\HTTP\RequestInterface $request, \CodeIgniter\HTTP\ResponseInterface $response, \Psr\Log\LoggerInterface $logger)
    {
        ...
        helper('common_functions_helper');
    ...
    public function cancelSubscription() {
        ...
        //     $user = auth()->user(); // get the current user object
        // $email = $user->email;
        // echo '>>>>>>>>>>> email >>>>> ' . $email; 

        // echo '>>>>>>>>>>> auth email >>>>> ' . auth()->user()->email; exit;
        ...
        if (! $this->validateData($inputData, $rules)) {
            ...
            try {
                ...
                // $user = $users->findByCredentials(['email' => auth()->id()->email ]);
                // $user_id = $user->id;

                if($oldSubscriptionData){
                    // pr($oldSubscriptionData);
                    ...
                    if(!empty($oldSubscriptionData['stripe_plan_id'])){
                        ...
                        if(!empty($packageInfo['description'])){
                            ...
                            // $activity = 'cancel the '.$packageInfo['description'].' subscription successfully';
                        }
                    }
                    $activity = 'successfully canceled your '.$packageName.' subscription';
                    if($subscription){
                        // return redirect()->to('/success')->with('message', 'Subscription created successfully');
                        $activity_data = [
                            'user_id'               => auth()->id(),
    ...
    public function handle_new()
    {
        ...
        try {
            ...
            $stripeWebhookEventModel->save([
                'event_id' => $event->id,
                'event_type' => $event->type,
                'payload' => json_encode($event->data->object), // Save event payload
            ]);
    ...
    public function processPendingEvents()
    {
        ...
        $events = $stripeWebhookEventModel
                        // ->select('id, event_id')
        ...
        foreach ($events as $event) {
            try {
                ...
                switch ($event['event_type']) {
                    ...
                    //     // log_message('info', "Payment succeeded: {$payload['id']}");
                    ...
                    case 'customer.subscription.created':
                        // Handle payment failure
                        // echo '>>>>>>>>>>>>>>>>>> handleCustomerSubscriptionCreated triggered>>>>>>>>>>>>>>>>> '; //exit;
                        // pr($payload);
                ...
                $stripeWebhookEventModel->update($event['id'], ['processed_at' => $processed_at ]);
                // $stripeWebhookEventModel->update($event['id'], ['processed_at' => date('Y-m-d H:i:s')]);
    ...
    public function handleInvoicePaymentFailed($invoice)
    {
        ...
        log_message('error', "Payment failed for customer $customerId.");
    }

    // customer.subscription.created
    public function handleCustomerSubscriptionCreated($subscription)
    {
        // echo '>>>>>>>>>>>>>>>>>> handleCustomerSubscriptionCreated >>>>>>>>>>>>>>>>> '; //exit;
        // echo '<pre>'; pr($subscription); echo '</pre>'; //exit;
        // echo '>>>>>>>>>>>>>>>>>> subscription data >>>>>>>>>>>>>>>>> '; //exit;
        ...
        // echo '>>>>>>>>>>>>>>>>>> planId >>>>>>>>>>>>>>>>> ' . $planId; //exit;
        // echo '>>>>>>>>>>>>>>>>>> subscription[id] >>>>>>>>>>>>>>>>> ' . $subscription['id']; exit;
        ...
        // if($subscription){
            ...
            // echo '<pre>'; pr($subscripData); echo '</pre>'; //exit;
            // echo '>>>>>>>>>>>>>>>>>> subscripData >>>>>>>>>>>>>>>>> ' ; exit;
    ...
    public function handleCustomerSubscriptionUpdated($subscription)
    {
        // Extract relevant data
    ...
    public function handleCustomerSubscriptionDeleted($subscription)
    {
        // Extract relevant data
        $subscriptionId = $subscription->id;
    ...
    public function handlePaymentIntentSucceeded($payment_intent)
    {
        // Extract relevant data
    ...
    public function handlePaymentMethodAttached($payment_method)
    {
        $adminMessage = '';
    ...
    public function handleCheckoutSessionCompleted($checkout_session)
    {
        // Extract relevant data
